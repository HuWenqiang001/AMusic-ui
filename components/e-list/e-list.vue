<template>
	<view class="list-container">
		...
		<scroll-view v-if="listData.length > 0" class="song-list" :scroll-y="true" @scroll="scroll">
			<view class="song-list-item" v-for="(item, index) in listData" :key="index" @click="play(item, 0)">
				<view class="item-container">
					<view>
						<view v-if="index == playIndex && currentTab == 4">
							<video :src="mvUrl" autoplay="true" :controls="false" :muted="true"></video>
						</view>
						<view v-else class="mv-cover" :style="{backgroundImage:`url(${item.cover})`}" @touchstart="mvHover(index)"></view>
					</view>
					<view>
						...
						<view class="singer-name">
							<text>{{`${item.artistName}\n播放${playCountConversion(item.playCount)}次`}}</text>
						</view>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		watch:{
			playIndex(newVal){
				this.getMvUrl(this.listData[newVal].id)
			}
		},
		onLoad() {
		...
		methods: {
			scroll(e) {
				// this.playIndex = Math.floor(e.detail.scrollTop/(e.detail.scrollHeight/this.listData.length - 10));
				this.playIndex = null;
			},
			mvHover(index){
				// console.log(index)
