	...
	<view class="special-list-container" :style="{height:height}">
		<view class="special-list-box">
			<view class="special-list-item" v-for="(item, index) in data.creatives" :style="{height: `calc(${height} * 0.9)`, minWidth: `calc(${height} * 0.63)`, maxWidth: `calc(${height} * 0.6)`}">
				...
				<image v-if="index > 0" :src="item.uiElement.image.imageUrl" mode="widthFix" @click="clicked(item.creativeId)"></image>
				<view class="scroll-item" v-if="index <= 0">
					<swiper class="scroll-item-swiper" :indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval"
						:duration="duration" :circular="circular" :vertical="vertical" @animationfinish="updataText(item.resources,$event)">
						<swiper-item class="scroll-item-swiper-item" v-for="(item, index) in item.resources" :key="index" @click="clicked(item.resourceId)">
