<template>
	<view class="list-container">
		...
		<scroll-view v-if="listData.length > 0" class="song-list">
			<view class="song-list-item" v-for="(item, index) in listData" :key="index" @click="play(item.resourceExtInfo.song, 1)">
				<view class="item-container">
					<view>
						<image :src="item.resourceExtInfo.song.al.picUrl" mode="heightFix"></image>
					</view>
					<view>
						<view class="song-name">
							<text :class="{playStatus: item.resourceExtInfo.song.id == songId}">{{item.resourceExtInfo.song.al.name}}</text>
						</view>
						<view class="singer-name">
							<text :class="{'playStatus-singer':item.resourceExtInfo.song.id == songId}">{{`${item.resourceExtInfo.song.ar.map(u => {return u.name}).join('/')}`}}</text>
						</view>
					</view>
					<view>
				...
				<u-line v-if="index < listData.length - 1"></u-line>
			</view>
	...
	export default {
		...
		methods: {
			...
			play(song, identification) {
				...
				if(identification === 1){
					// getApp().globalData._songData.shift(song);
				}
				// this.songId = song.id;
			}
		}
